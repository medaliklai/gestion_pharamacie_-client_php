...
if (!isset($_SESSION['role']) or $_SESSION['role']!="client")
{
	...
	die();
		...
		$receive=$rep->rowCount();
			...
			$nom=$_SESSION['nom'];
		$requete=$base->prepare('SELECT * from message where id_emetteur=? and emetteur=? and role_emetteur="client"');
...
while($row=$rep->fetch())
		...
		<div id="affiche">
			...
			<a href="client_supprimer_message.php?id_msg=<?=$row['id_msg'];?> " class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span></a>
		</form>
		</div>
...
while($data=$requete->fetch())
		...
		<div id="affiche">
			...
			<form method="POST" action="#">
			<a href="client_supprimer_message.php?id_msg=<?=$data['id_msg'];?> " class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span></a>
		</form>
		</div>
